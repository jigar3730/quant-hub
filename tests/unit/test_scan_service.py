...
def test_scan_service_dry_run_persists(tmp_path, monkeypatch):
    monkeypatch.setattr(
        "quant_hub.application.scan_service.DEFAULT_OUTPUT_CSV",
        tmp_path / "out.csv",
    )
    monkeypatch.setattr(
        "quant_hub.application.scan_service.DEFAULT_OUTPUT_JSON",
        tmp_path / "out.json",
    )
    ...
    report = service.scan_repo.load_report(universe_id="custom")
