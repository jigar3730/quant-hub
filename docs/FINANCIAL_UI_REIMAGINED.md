...
| Heatmap / funnel charts | **Plain divs, not Plotly** (reversed 2026-09-12) | `npm audit` found a critical XSS vulnerability (`maplibre-gl`, transitive via `plotly.js`) with no safe version combination — `react-plotly.js` needs `plotly.js >=3.0.0`, every `plotly.js >=2.35.0` depends on the vulnerable range. `ScanFunnel`/`CommandCenterHeatmap` both shipped as plain divs instead; revisit only if a genuinely complex chart type (not a grid or bar) makes a library unavoidable — check `npm audit` again first regardless of which one |
...
3. **Compare view** (§3.5) — ✅ **Done.** `CompareTickers`, up to 4 tickers, aligned factor bars per column rather than a radar/spider chart. (The original note here said Plotly would eventually justify itself for item #7 — it didn't; see #7 below. A radar chart is still a real gap nothing here covers, if ever wanted.)
...
5. **Scan-run funnel view** (§2.1) — ✅ **Done.** `ScanFunnel`, plain div bars (universe → eligible, tier breakdown, filter-reason breakdown), wired into Universe Explorer alongside `RegimeBanner`.
6. **MetricGrid generalization** (§1.4) — ✅ **Done.** `orderedScoreKeys()`/`scoreLabel()` in `lib/scoring.ts`: a new Launchpad factor now shows up automatically in `UniverseTable`/`TickerTechnicalCard` instead of being silently dropped, with a humanized fallback label if uncurated. `CompareTickers` deliberately left on the fixed `SCORE_ORDER` — see its own code comment for why (per-column generalization risked misaligning columns across tickers).
7. **Command Center heatmap** — ✅ **Done.** `CommandCenterHeatmap`, a strategy x universe coverage grid on the Today's Priorities page. Built without Plotly after `npm audit` found an unavoidable critical vulnerability in its dependency chain — see §4.2. Shows every known universe (not just ones with coverage today), so gaps are visible, not just where signal exists.
8. **Real Watchlist** (§3.5) — 🔶 **Needs careful design, not high priority right now.** The first item in this sequence requiring new backend *persistence* (a table, write endpoints) rather than read-only additive work — a genuinely different risk category than everything shipped through item #7. Scope the data model and write-boundary explicitly before touching code, same rigor as the Phase 1 API boundary got.
9. **Dark-first palette pass** — low priority, but **not** high-risk — pure CSS token value changes on an architecture already built to support them (§4.1). Do once several more screens exist so it themes consistently rather than piecemeal.
10. **"Actionable Signals" ML cross-reference** — 🔶 **Needs careful design.** Mixing historical outcome data with live ranked signals risks implying predictive/backtested claims the data doesn't actually support if the historical-vs-live framing isn't handled deliberately (§3.2).
11. **Entry/stop-level logic** — 🔴 **Needs careful design — the highest-risk item on this list.** Explicitly **not** scheduled here; needs its own risk-methodology conversation, with the user's explicit sign-off on the actual methodology, before any UI is built for it. Fabricated stop-loss/entry numbers a user might act on is real financial harm, not a styling decision.
