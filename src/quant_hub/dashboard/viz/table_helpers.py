...
from quant_hub.dashboard.viz.navigation import finviz_quote_url

TICKER_LINK_HELP = "Opens the quote on Finviz."
...
def ticker_link_value(ticker: str) -> str:
    symbol = ticker.strip().upper()
    return f"{finviz_quote_url(symbol)}#{symbol}"


def with_ticker_links(df: pd.DataFrame) -> pd.DataFrame:
    """Add ``ticker_link`` URL column; keep ``ticker`` for selection and exports."""
    if df.empty or "ticker" not in df.columns:
        return df
    out = df.copy()
    out["ticker_link"] = (
        out["ticker"].astype(str).str.strip().str.upper().map(ticker_link_value)
    )
    return out
